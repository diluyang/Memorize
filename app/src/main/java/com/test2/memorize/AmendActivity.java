...
public class AmendActivity extends BaseActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.amend_linear_item);
        init();
    ...
    void init(){
        ...
        if (intent!=null){
            ...
            record.setId(Integer.valueOf(intent.getStringExtra(MyDB.ID)));
            record.setTitleName(intent.getStringExtra(MyDB.TITLE));
            record.setTextBody(intent.getStringExtra(MyDB.BODY));
            record.setCreateTime(intent.getStringExtra(MyDB.TIME));
    ...
    boolean updateFunction(String body){
        ...
        if(flag){
            ...
            values.put(MyDB.TIME,getNowTime());
            db.update(MyDB.TABLE_NAME,values,MyDB.ID+"=?",
